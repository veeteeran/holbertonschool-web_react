...
describe('state of App', () => {
  ...
  it('checks the state for displayDrawer is true when handleDisplayDrawer method called', () => {
    const wrapper = mount(<App />);
  ...
  it('checks the state for displayDrawer is false when handleHideDrawer method called', () => {
    const wrapper = mount(<App />);
  ...
  it('checks that markNotificationAsRead works as intended', () => {
    ...
    expect(wrapper.state().listNotifications).toEqual([{ id: 2, type: 'urgent', value: 'New resume available' }])
  });
});
